...
func WaitForPodsRunning(cl *cluster.Cluster, kubeconfigFilePath, namespace, selector string, replicas int) error {
	...
	log.Warnf("Waiting for pods to be running. label: %q, namespace: %q, replicas: %v, duration: %v, cluster: %s.", selector, namespace, replicas, constants.WaitDurationResources, cl.Name)
	podsContext, cancel := context.WithTimeout(ctx, constants.WaitDurationResources)
	wait.Until(func() {
		...
		if err == nil && len(podList.Items) == replicas {
			...
			log.Debugf("Still waiting for pods. label: %q, namespace: %q, replicas: %v, duration: %v, cluster: %s.", selector, namespace, replicas, constants.WaitDurationResources, cl.Name)
		}
...
func WaitForDeployment(cl *cluster.Cluster, kubeconfigFilePath, namespace, deploymentName string) error {
	...
	ctx := context.Background()
	log.Infof("Waiting up to %v for %s deployment roll out %s...", constants.WaitDurationResources, deploymentName, cl.Name)
	deploymentContext, cancel := context.WithTimeout(ctx, constants.WaitDurationResources)
	wait.Until(func() {
		...
		if err == nil {
			if deployment.Status.ReadyReplicas == *deployment.Spec.Replicas {
				...
				log.Warnf("Still waiting for %s deployment for %s, ready replicas: %v", deploymentName, cl.Name, deployment.Status.ReadyReplicas)
			}
		} else {
...
func WaitForDaemonSet(cl *cluster.Cluster, kubeconfigFilePath, namespace, daemonSetName string) error {
	...
	ctx := context.Background()
	log.Infof("Waiting up to %v for %s daemon set roll out %s...", constants.WaitDurationResources, daemonSetName, cl.Name)
	deploymentContext, cancel := context.WithTimeout(ctx, constants.WaitDurationResources)
	wait.Until(func() {
		...
		if err == nil && daemonSet.Status.CurrentNumberScheduled > 0 {
			if daemonSet.Status.NumberReady == int32(len(nodeList.Items)) {
				...
				log.Warnf("Still waiting for %s daemon set roll out for %s, ready replicas: %v", daemonSetName, cl.Name, daemonSet.Status.NumberReady)
			}
		} else {
